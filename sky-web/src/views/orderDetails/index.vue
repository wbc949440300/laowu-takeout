  ...
  <div class="dashboard-container">
    ...
    >
      <el-scrollbar style="height: 100%">
        ...
        <div class="order-timeline" v-if="timeline.length">
          <div class="amount-label">订单进度</div>
          <el-timeline>
            <el-timeline-item v-for="item in timeline" :key="item.id" :timestamp="item.createTime">
              {{ timelineText(item.eventType) }}<span v-if="item.remark">：{{ item.remark }}</span>
            </el-timeline-item>
          </el-timeline>
        </div>
      </el-scrollbar>
...
export default class extends Vue {
  ...
  async goDetail(id: any, status: number, row?: any) {
    ...
    this.orderId = id
    const [{ data }, timelineResponse]: any = await Promise.all([
      queryOrderDetailById({ orderId: id }),
      queryOrderTimeline(id),
    ])
    this.diaForm = data.data
    this.timeline = timelineResponse.data && timelineResponse.data.data ? timelineResponse.data.data : []
  ...
  timelineText(type: string) {
    return ({ PLACED: '已下单', PAID: '已支付', CONFIRMED: '商家已接单', DELIVERING: '配送中', COMPLETED: '已完成', CANCELLED: '已取消', REFUND_APPLY: '已申请退款' } as any)[type] || type
  }
